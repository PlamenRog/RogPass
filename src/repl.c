...
uint32_t select_entry(EntryPass* entries, uint32_t numEntries, const char* keyword) {
	...
	while (true) {
        ...
        printf("Enter the number of the entry which you'd like to %s: ", keyword);
        if (scanf("%u", &num) != 1) {
            while(getchar() != '\n');
...
int main(){
	...
	while(true) {
		...
		if (strcmp(choice, "CREATE") == 0 || strcmp(choice, "C") == 0) {
			...
			// 5 characters less, because .rogp will be appended
			scanf(" %123[^\n]", filename);

			// Check if the file contains an extention
